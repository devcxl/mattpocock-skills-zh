...
依赖关系以明确的指令来表达：**调用 Skill 工具**并传入指定的技能名（`Call the Skill tool with "grilling"`），而不是深层的 `../other-skill/FILE.md` 交叉引用，也不是留给模型自行解读的裸 `/skill` 式提及。点名工具才能让它真正被触发：多数 harness 把技能调用暴露为模型可调用的工具，明说这一点比在自然语言里丢一个 `/name` 然后指望它被当作命令的命中率高得多。去掉开头的 `/` 也让写法保持 harness 中立——单独的技能名不携带任何关于它属于哪个 harness 触发语法的假设。共享参考文档归所属技能所有；其他技能通过调用 Skill 工具并传入该技能来获取内容，而不是跨文件夹链接。

这指的是**操作性**指令——技能自身的步骤要求 agent 立即去运行另一个技能。路由性质的文案只是列出技能供人类挑选（`ask-matt`、分类 `README.md`）并不构成调用，因此保留 `/skill` 风格的名字作为普通标签。

Skill 工具一次只接收一个技能。一个步骤需要两个技能就是两次调用，而不是一次调用带两个名字——要明说（`Call the Skill tool twice, for "grilling" and "domain-modeling"`），而不是写成“用 X 和 Y 调用它”，那会被读成一次调用同时接收两者。

整套约定只在被点名的技能是**模型调用**时才成立。用户调用的技能永远无法通过这种方式触达——根据上面的不变式，没有其他技能可以调用它，包括向 Skill 工具点名它也不行。当一个步骤的前置条件是用户调用的技能（如 `setup-matt-pocock-skills`）时，把它表述为让人类去执行的指令——“告诉用户运行 `/setup-matt-pocock-skills`”——而绝不是一个 Skill 工具调用。
