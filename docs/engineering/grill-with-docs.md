...
有状态的输出假设由单个人来维护。一个两人团队在一个仓库里跑了四个月，报告大约 20% 的抽样已合并 PR 出现状态漂移，其中 ADR 引用和 README 声明是漂移最严重的表面——刻意、人工维护的文档比代理记忆漂得更厉害。修剪过时的文档守不住；同样的清理几天之内又过时了。真正有效的是彻底删除影子状态，并在 CI 里加一个确定性的引用与链接 linter。

相关的一点：在一个仓库里对互不相关的变更反复运行这个技能，往往会积累出话题混杂的文档，因为没有任何东西把一次会话的输出与另一次的分开。这两点今天都还没在技能里修复。
