...
**循环论证测试（Tautological tests）**将实现过程重述在断言中，因此它们构造性通过，却不提供任何信心。当期望值以与代码相同的方式计算时——`expect(add(a, b)).toBe(a + b)`、对你通过手工以同样方式推导的数值进行快照、断言常量等于自身——测试永远不可能与代码不一致：破坏代码，断言也会跟着以同样的方式破坏。期望值必须来自独立的真实来源——已知正确的字面量、手工验证的示例或规格说明。
