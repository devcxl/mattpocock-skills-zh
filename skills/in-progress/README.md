# 开发中

仍在开发中的技能。尚未达到发布标准——可能存在粗糙的边界、破坏性变更和被放弃的实验。在毕业到稳定版之前，这些技能不会包含在插件和顶层 README 中。

- **[loop-me](./loop-me/SKILL.md)** — 通过多次会话将自己盘问出可实施的工作流规格说明，使用当前目录作为有状态工作空间。用户调用。
- **[writing-shape](./writing-shape/SKILL.md)** — 将文档或消息的片段组织到 markdown 大纲中：选择合适的部分，安排顺序，合并或拆分，添加必要的过渡段落。
- **[writing-beats](./writing-beats/SKILL.md)** — 在节奏和空间方面思考写作：节拍、动作、感官细节、叙事弧在写作之前就已确定。
- **[writing-fragments](./writing-fragments/SKILL.md)** — 从各种来源拼凑出文档的骨架，填补空白，整合反馈，然后打磨成连贯的草稿。
- **[wizard](./wizard/SKILL.md)** — 指导用户完成可能需要的所有中间步骤，然后再将工作交给 agent 来完成。
- **[claude-handoff](./claude-handoff/SKILL.md)** — 使用 `claude --bg` 创建一个专门的后台 agent，用于处理独立的子问题。
- **[setup-ts-deep-modules](./setup-ts-deep-modules/SKILL.md)** — 为 TypeScript 项目配置模块深度工具链：安装并配置 dependency-cruiser 规则（规则集、入口点、已知的"流氓"模块别名），然后建立跨接口的模块边界，并修正任何检测到的违规。
- **[to-questionnaire](./to-questionnaire/SKILL.md)** — 将一个你无法完全回答的决策转化为 Markdown 问卷，让别人异步填写，或在会议上一起完成。它盘问的是关于**发送**的问题（发给谁、需要什么回来），而不是主题本身。用户调用。
