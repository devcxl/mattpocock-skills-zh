...
当一个松散的想法需要不止一个 Agent 会话才能转化为计划时，会调用此技能。它在一个 Markdown 文件中创建一个有状态的决策映射（decision map），并引导用户依次处理一系列 Ticket，以解决悬而未决的问题——这些问题可能涉及原型开发、调研或盘问。
...
当某个 Ticket 的 `Blocked by` 列表中所有 Ticket 均为 `resolved` 状态时，该 Ticket **解除阻塞（unblocked）**。会话在开始工作前，应先**认领（claim）**其 Ticket：将 `Status` 设为 `in-progress` 并保存映射，以便并发会话跳过它。
...
用户传入现有映射的路径来发起调用。Ticket 编号**可选**——不传编号时，由你选择下一个决策，而非用户。

1. 将**整个映射**加载为上下文。
2. 选择 Ticket。如果用户指定了编号，使用它。否则选择编号最小的 `open` 且已[解除阻塞](#结构)的 Ticket。[认领它](#结构)：将 `Status` 设为 `in-progress` 并在任何工作开始前保存。
3. 解决它，根据需要调用其他技能。如有疑问，使用 `/grilling` 和 `/domain-modeling`。
...
> **Next steps** — 3张 Ticket 已解除阻塞: #4, #5, #6.
> 清理上下文，然后打开新的会话。
>
...
> **并行** — 每行一个窗口，最多 3 个：
> ```
> 调用 /decision-mapping，映射位于 <path>，Ticket #4。
> 调用 /decision-mapping，映射位于 <path>，Ticket #5。
> 调用 /decision-mapping，映射位于 <path>，Ticket #6。
> ```
